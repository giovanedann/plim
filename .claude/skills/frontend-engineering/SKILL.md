...
Routes live in `src/routes/` and are auto-generated by the Vite plugin:

```
src/routes/
├── __root.tsx              # Root layout (Toaster, DevTools)
├── index.tsx               # "/" → redirect based on auth
├── _auth.tsx               # Auth layout group (guest-only)
├── _auth/
│   ├── sign-in.tsx         # /sign-in
│   └── sign-up.tsx         # /sign-up
├── _app.tsx                # App layout group (authenticated)
├── _app/
...
| `_layoutName.tsx` | Layout group (not in URL) | `_auth.tsx`, `_app.tsx` |
| `$param.tsx` | Dynamic segment | `$userId.tsx` → `/123` |
| `$.tsx` | Catch-all (404) | Matches any unmatched path |

### Route Guards with `beforeLoad`

Use `beforeLoad` for authentication checks instead of guard components:

```typescript
// routes/_app.tsx - Protected routes
...
// routes/_auth.tsx - Guest-only routes
...
// Type-safe links - typos caught at compile time
...
### Search Params (Type-Safe with Zod)
...
function ExpensesPage() {
  const { month, category } = useSearch({ from: '/_app/expenses' })
...
Auth state is passed via router context for use in `beforeLoad`:
...
// Provider passes actual values
<RouterProvider
  router={router}
  context={{ auth: { user, isInitialized } }}
/>
